...
public class TCLParser {
	...
	public static void readBracketExpression(PushbackReader s, StringBuilder b) throws IOException {
		...
		while ( brackets > 0 ) {
			...
			else if ( c == '{' ) readCurlyBraceString(s, b);
	...
	public static Parameter evaluate(String cmd, Context ctx) {
		Parameter[] pargs = TCLParser.parseLine(new PushbackReader(new StringReader(cmd)), ctx);
		for ( Parameter p : pargs ) {
			ctx.sendDebugMessage("[P] " + p.toString());
		}
		
		return evaluate(pargs, ctx);
	}
	...
	public static Parameter[] parseLine(PushbackReader s, Context ctx) {
		...
		try {
			while ( (r = s.read()) > 0) {
				...
				if ( in == '"' ) {
					if ( c == '"' ) in = '\0';
					else if ( c == '{' ) { s.unread(r); readCurlyBraceString(s, current);  }
					else if ( c == '[' ) { s.unread(r); currentp = evaulateBracketExpression(s, ctx); }
					else if ( c == '$' ) { s.unread(r); currentp = evaulateVariable(s, ctx); }
					else append = true;
				} else {
					if ( c == '"' ) in = c;
					else if ( c == '{' ) { 
						s.unread(r); readCurlyBraceString(s, current); 
					} else if ( c == '[' ) { 
						s.unread(r); 
				...
				if ( append ) {
					...
					if ( c == '\\' ) current.append(translateSlashCode(s.read()));
